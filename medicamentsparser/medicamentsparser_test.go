...
func TestTSVPresentationsEdgeCases(t *testing.T) {
	...
	testCases := []struct {
		...
		expectedPrixPublic *float64 // nil = absent price (JSON null)
	...
	}{
		{
			...
			checkPrices:        true,
			expectedPrix:       24.34,
			expectedPrixPublic: f64Ptr(25.50),
		...
		},
		{
			...
			checkPrices:        true,
			expectedPrix:       0,
			expectedPrixPublic: nil,
			expectedHonoraires: nil,
			description:        "Empty prix parses as 0 (legacy contract until sunset); prixPublic and honoraires stay nil (JSON null)",
		},
		{
			...
			expectedPrixPublic: f64Ptr(2551.50),
			expectedHonoraires: f64Ptr(0.50),
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.checkPrices && len(result) > 0 {
				...
				if r.Prix != tc.expectedPrix ||
					!f64PtrEqual(r.PrixPublic, tc.expectedPrixPublic) ||
					!f64PtrEqual(r.HonorairesDispensation, tc.expectedHonoraires) {
					t.Errorf("Expected [prix=%v, prixPublic=%v, honoraires=%v], got [prix=%v, prixPublic=%v, honoraires=%v]. Description: %s",
						tc.expectedPrix, tc.expectedPrixPublic, tc.expectedHonoraires,
						r.Prix, r.PrixPublic, r.HonorairesDispensation, tc.description)
				}
			}
